    ...
    operations = [
        ...
        migrations.CreateModel(
            ...
            fields=[
                ("id", models.BigAutoField(primary_key=True, serialize=False)),
                (
                    "question_is_single_choice",
                    models.BooleanField(
                        default=False,
                        editable=False,
                        help_text="Denormalized copy of question.question_type == 'single_choice'. Backs the partial unique constraint that enforces one vote per user per single-choice question at the database level, because a UniqueConstraint condition cannot reference fields on the joined SurveyQuestion. Populated in save() and can go stale only if a question's question_type is edited after votes exist; surveys are designed to be immutable, so this is not expected to happen.",
                    ),
                ),
        ...
        migrations.AddConstraint(
            ...
            constraint=models.UniqueConstraint(
                fields=("user", "choice"), name="one_vote_per_user_per_choice"
            ),
        ),
        migrations.AddConstraint(
            model_name="surveyvote",
