...
class Command(BaseCommand):
	...
	def handle(self, *args: Any, **options: Any) -> None:
		...
		while True:
			...
			while True:
				...
				if not choice_text:
					if not choices_list:
						self.stdout.write(self.style.ERROR("At least one choice must be provided"))
						continue
					break
		...
		if not questions:
			self.stdout.write(self.style.WARNING("No questions added. Survey creation cancelled."))
			return
	...
	def _create_survey(
		...
		try:
			...
			self.stdout.write(
				self.style.SUCCESS(f"✓ Created survey: {survey.title} (ID: {survey.id})")
			)
			...
			for question_order, question_data in enumerate(questions, start=1):
				...
				self.stdout.write(
					self.style.SUCCESS(f"✓ Created question {question_order}: {question.text}")
				)
				...
				for choice_order, choice_text in enumerate(question_data['choices'], start=1):
					...
					self.stdout.write(
						self.style.SUCCESS(f"  ✓ Added choice #{choice_order}: {choice.text}")
					)
			...
			raise CommandError(f"Failed to create survey: {str(exception)}") from exception
